    ...
    public function MailToPost()
    {
        ...
        foreach ($this->data as $d) {
            switch ($d['type']) {
                ...
                case "text":
                    if ($this->post_parameters['type'] == "text") {$textField = "body";} else {$textField = "caption";}

                        if (!empty($this->post_parameters[$textField])) {$this->post_parameters[$textField] .= "<br />" . $d['data'];}	// On rajoute à la suite
